...
const cancelMasterClass = async () => {
    console.log(masterClasstoCancel.value.id)
    handleOpen();
  ...
  <Modal :open="openModalUpdate" @close="handleOpenModalUpdate">
    ...
    <form @submit.prevent="updateMasterClass">
        ...
        <div>
          ...
          <input type="text" name="title" id="title" v-model.trim="masterClasstoUpdate.title" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500" placeholder="Title">

        </div>
        <div>
          <label for="description">Description</label>
          <textarea name="description" id="description" v-model.trim="masterClasstoUpdate.description" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500" placeholder="Description"/>
        </div>
          
        <div class="flex justify-between">
          <div>
            <label for="time">Time</label>
            <input type="time" name="time" id="time" v-model="masterClasstoUpdate.time" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500">
          </div>
          <div>
            <label for="date">Date</label>
            <input type="date" name="date" id="date" v-model="masterClasstoUpdate.date" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500">
          </div>
        ...
        <div class="flex justify-between">
          <div>
            ...
            <input type="text" name="adress" id="adress" v-model.trim="masterClasstoUpdate.adress" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500" placeholder="Adress">
          </div>
          <div>
            <label for="city">City</label>
            <input type="text" name="city" id="city" v-model.trim="masterClasstoUpdate.city" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500" placeholder="City">
          </div>
        </div>
        <div class="flex justify-between">
          <div class="mr-2">
            <label for="postalCode">Postal Code</label>
            <input type="text" name="postalCode" id="postalCode" v-model.trim="masterClasstoUpdate.postalCode" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500" placeholder="Postal Code">
          </div>
          <div>
            <label for="country">Country</label>
            <input type="text" name="country" id="country" v-model.trim="masterClasstoUpdate.country" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500" placeholder="Country">
          </div>
        </div>
  ...
  <div>
      <main>
          <div class="relative pt-16 pb-32 flex content-center items-center justify-center min-h-screen-75">
              <div class="absolute top-0 w-full h-full bg-center bg-cover" style="background-image: url('https://www.marmiteauxplumes.com/wp-content/uploads/2021/01/36519-738-cupcakes-5116009_640.jpg');">
                      ...
                      <div class="w-full lg:w-6/12 px-4 ml-auto mr-auto text-center">
                          <div class="pr-12">
                              ...
                              <p class="mt-4 text-lg text-blueGray-200">
                                Welcome to our platform, where our pastry cooks offer you pastry workshops, an opportunity for enthusiasts to learn how to bake and unearth the secrets of our experts.
                              </p>
                              <button class="bg-red-500 mt-2 text-white active:bg-red-600 font-bold uppercase text-sm px-6 py-3 rounded-full shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-15" @click="addMasterClass">Add Master Class</button>
                          </div>
                      </div>
                  </div>
              </div>
                  ...
                  <div class="flex flex-wrap">
                      ...
                      <div v-for="masterClass in masterClasses" class="w-full md:w-4/12 px-4 mr-auto ml-auto  pt-6  text-center">
                        <div class="relative flex flex-col min-w-0 break-words  w-full mb-6 shadow-lg rounded-lg bg-red-300">
                          <img alt="..." src="https://static.750g.com/images/1200-630/2bb28ae83807e3f46e861587586c6aee/adobestock-182827481.jpeg" class="w-full align-middle rounded-t-lg" />
                            <blockquote class="relative p-8 mb-4">
                              ...
                              <div v-if="!masterClass.isCanceled">
                                <button @click="getMasterClassToCancel(masterClass)" class="bg-red-500 mt-2 text-white active:bg-red-600 font-bold uppercase text-sm px-6 py-3 rounded-full shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-15" type="button">
