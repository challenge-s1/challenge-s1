...
const openModalCommentUpdate = ref(false);
// const openModalCommentDelete = ref(false);
const openModalCommentReport = ref(false);
const today = new Date();
const commentContent = ref('');
const commentToUpdate = reactive({
    id: '',
    content: ''
});
const reasonsList = ref([])
const commentToReport = reactive({
    id: '',
    reason: ''
});
...
const handleOpenModalCommentUpdate = () => {
    openModalCommentUpdate.value = !openModalCommentUpdate.value;
};

const handleOpenModalCommentReport = () => {
    openModalCommentReport.value = !openModalCommentReport.value;
};
...
const getCommentToUpdate = async (comment) => {
    commentToUpdate.id = comment.id;
    commentToUpdate.content = comment.content;
    handleOpenModalCommentUpdate();
}

// const getCommentToDelete = async (comment) => {
//     commentToDelete.id = comment.id;
//     commentToDelete.content = comment.content;
//     handleOpenModalCommentDelete();
// }
...
<template>
    ...
    <Modal :open="openModalUpdate" @close="handleOpenModalUpdate">
        ...
        <form @submit.prevent="updateMasterClass">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" v-model.trim="masterClass.title"
            ...
            <div>
                ...
                <textarea name="description" id="description" v-model.trim="masterClass.description"
                    class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500"
            ...
            <div class="flex justify-between">
                <div>
                    ...
                    <input type="time" name="time" id="time" v-model="masterClass.time"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500">
                </div>
                <div>
                    <label for="date">Date</label>
                    <input type="date" name="date" id="date" v-model="masterClass.date"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500">
                </div>
            ...
            <div class="flex justify-between">
                <div>
                    ...
                    <input type="text" name="adress" id="adress" v-model.trim="masterClass.adress"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500"
                        placeholder="Adress">
                </div>
                <div>
                    <label for="city">City</label>
                    <input type="text" name="city" id="city" v-model.trim="masterClass.city"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500"
                        placeholder="City">
                </div>
            </div>
            <div class="flex justify-between">
                <div class="mr-2">
                    ...
                    <input type="text" name="postalCode" id="postalCode" v-model.number="masterClass.postalcode"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500"
                        placeholder="Postal Code">
                </div>
                <div>
                    <label for="country">Country</label>
                    <input type="text" name="country" id="country" v-model.trim="masterClass.country"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500"
                        placeholder="Country">
                </div>
            </div>
    ...
    <Modal :open="openModalCommentReport" @close="handleOpenModalCommentReport">
        <template #title>
            <h1 class="text-2xl font-semibold text-gray-700">Report Comment</h1>
        </template>
    ...
    <Modal :open="openModalCommentUpdate" @close="handleOpenModalCommentUpdate">
        <template #title>
            <h1 class="text-2xl font-semibold text-gray-700">Update Comment</h1>
        </template>
        <form @submit.prevent="updateComment">
            <div>
                <label for="description">Description</label>
                <textarea name="description" id="description" v-model.trim="commentToUpdate.content"
                    class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500"
                    placeholder="Description" />
            </div>
            <button type="submit"
            ...
            <div class="items-center flex flex-wrap">
                <div class="w-full lg:w-6/12 px-4 ml-auto mr-auto text-center">
                    <div class="">
                        ...
                        <p v-if="masterClass.owner != null" class="mt-4 text-lg text-blueGray-200">
        ...
        <div class="flex flex-wrap">
            <div class="w-full px-4 flex-1">
                <div class="flex">
                    ...
                    <span class="text-sm font-bold block my-4 p-3 text-blueGray-700 rounded">{{ masterClass.adress }} {{
                        masterClass.postalcode
                    }} {{ masterClass.city }}, {{ masterClass.country }}</span>
            ...
            <div class="w-full px-4 flex-1">
                <div class="flex">
                    ...
                    <span class="text-4XL text-center font-bold block my-4 p-3 text-blueGray-700 rounded">{{
                        masterClass.maxNumber
                    }} seats available</span>
                </div>

            </div>
            <div v-if="masterClass.owner != null && store.getters.user && masterClass.owner.id == store.getters.user.id"
                class="w-full px-4 flex-1">
                <button v-if="masterClass.isCanceled" disabled
            ...
            <span
                ...
                <div v-else class="flex mt-8">
                    <svg @click="getMasterClassToUpdate" xmlns="http://www.w3.org/2000/svg" fill="none"
                        viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                    </svg>
                    <svg @click="getMasterClassToCancel" xmlns="http://www.w3.org/2000/svg" fill="none"
                        ...
                        <div class="max-w-2xl mx-auto px-4">
                            ...
                            <form @submit.prevent="addComment" class="mb-6">
                                <div
                                    ...
                                    <textarea id="comment" rows="6" v-model="commentContent"
